...
void solve() {
	...
	for (int i = 0 ; i < n; i++) {
		...
		if (a[i] % 2 == 1) can_odd =true;
		else can_even = true;
		
		if (a[i] - odd > 0){
			if ( (a[i] - odd) % 2 == 0) can_even = true;
			else can_odd = true;
		}
		if (a[i] - even > 0) {
			if ( (a[i] - even) % 2 == 0) can_even = true;
			else can_odd = true;
		}
		
		
		all_even = all_even &&  can_even;
		all_odd = all_odd && can_odd;
