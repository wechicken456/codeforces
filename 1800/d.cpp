...
bool check (int i, int j, int n) {
	for (int c = 0 ; c < 26; c++) {
		if (pref[i-1][c] + pref[n][c] - pref[i+1][c] != pref[j-1][c] + pref[n][c] - pref[j+1][c]) {
			return false;
		}
	}
	return true;
...
void solve() {
	...
	cin >> s;
	int sim = 0;
	pref = vector<vector<int>>(n + 2, vector<int>(26, 0));
	for (int i = 0 ; i< n; i++) {
		for (int j = 0 ; j < 26 ;j++) {
			pref[i+1][j] = pref[i][j];
		}
	...
	for (int i = 0 ;i  <26; i++) pref[n+1][i] = pref[n][i];
	for (int i = 1 ; i <= n - 1; i++) {
		int j = i + 1;
		while (j <= n - 1 && check(i, j, n)) {
			j++;
		}
		sim += j - i - 1;
		i = j - 1;
