...
void solve() {
	...
	ll task_days = (ll)( (n-1) / 7) + 1;
	ll double_need = ceil((long double)P / (2*t + l));
	ll rem_double = double_need - (ll)(task_days / 2);
	ll extra = task_days % 2;
	
	if (rem_double <= 0) {
		cout << n - double_need << "\n";
	}
	else {
		ll need_amount = P - ((ll)(task_days / 2)) * (2*t + l);
		n -= (ll)(task_days / 2);
		if (extra == 1) {
			need_amount -= (t + l);
			n -= 1;
		...
		if (need_amount <= 0) {
			...
			n = n - (ll)(ceil((double)need_amount / l));
			cout << n << "\n";
		}
	}
}
