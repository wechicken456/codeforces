...
void solve() {
	...
	for (int i = 1 ; i < n; i++) {
		cin >> a[i];
		if (close[i-1].second > a[i] - a[i-1]) {
			close[i-1] = {i, a[i] - a[i-1]};
		}
		close[i] = {i - 1, a[i] - a[i-1]};
	}
	for (int i = 1; i < n ; i++){
		if (close[i].first == i - 1) {
			l[i] = l[i-1] + 1;
		}
		else {
			l[i] = 0;
	...
	vector<int>pref_l(n + 1, 0);
	pref_l[0] = l[0];
	for (int i = 1 ; i < n; i++) {
		if (l[i] == 0) {
			pref_l[i] = pref_l[i - 1] + a[i] - a[i-1];
		}
		else {
			pref_l[i] = pref_l[i-1] + 1;
		}
	}
	vector<int>pref_r(n + 1, 0);
	pref_r[n-1] = 0;
	for (int i = n-2 ; i >= 0; i--) {
		if (r[i] == 0) {
			pref_r[i] = pref_r[i + 1] + a[i + 1] - a[i];
		}
		else {
			pref_r[i] = pref_r[i+1] + 1;
		}
	}
	int q;
	cin >> q;
	for (int i = 0 ;i <  q; i++) {
		...
		if (x > y) {
			cout << abs(pref_l[x] - pref_l[y]) << "\n";
		}
		else {
			cout << abs(pref_r[y] - pref_r[x] )<< "\n";
		}
	}
}
