...
void solve() {
	...
	int cnt = 0;
	while (right >= left) {
		int mul = min(x, right - idx + 1);
		if (mul < 0 ) mul = 0;
		int cur = pref[right] - pref[left] + a[left] - (pref[right] - pref[right - mul]);
		int nxt = pref[right] - pref[left] + a[left] - (pref[right] - pref[right - mul]);
