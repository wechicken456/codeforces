...
void solve() {
	...
	double avg = sum / n;
	sort(a.begin(), a.end());
	int cnt = lower_bound(a.begin(), a.end(), avg / 2) - a.begin();
	if (cnt > (int)(n / 2)) {
		cout << "0\n";
		return;
	}
	else {
		double diff = a[(int)(n / 2)] - avg / 2;
		ll add = ceil(diff*(double)n*(double)2);
		if ((double)((avg + (double)add / n) / 2) <= a[(int) n / 2]) add++;
		cout << add << "\n";
	}
}
