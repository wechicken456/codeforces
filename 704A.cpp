...
int main(){
	...
	int n , q;
	cin >> n >> q;
	vector<int>cur_head(n + 1, 0);
	vector<int>app;
	app.pb(0);
	vector<pair<int, vector<int>>>pos(n + 1);
	int ans = 0, max_prev_3 = 0, cnt_1 = 1;
	for (int j = 0 ; j < q; j++) {
		int t, x;
		cin >> t >> x;
		if (t == 1) {
			...
			for (int k = max_prev_3 ; k <= x ; k++) {
				int i = app[k];
				int idx = upper_bound(pos[i].second.begin() + cur_head[i], pos[i].second.end(), x) - pos[i].second.begin();
				int tmp = idx - cur_head[i];
				cur_head[i] = idx;
				ans -= tmp;
			}
			max_prev_3 = x;
